...
#include "Kc.h" //Matriz de Calibración Calculada. Edit By DPM
...
//extern esphome::uart::UARTComponent *uart_bus;
...
namespace esphome {
  namespace as7341 {
    ...
    //ATIME define los ciclos de iteración
    short AS7341Component::setIntegrationTime(uint16_t integrationTime) {
      uint8_t bit = integrationTime;
      if (this->write_register(0x81, &bit, 1) != i2c::ERROR_OK){
        ESP_LOGE(TAG, "Error escribiendo tiempo de integracion!");
        return -1;
      }
      return 0;
    }

    //Echarle un ojito a esta parte del código
    //ASTEP define la duración de cada ciclo. Tiene dos cadenas 0xCA y 0xCB para la escritura.
    short AS7341Component::setIntegrationSteps(uint16_t integrationSteps) {
      uint8_t low  =  integrationSteps & 0xFF;
      uint8_t high = (integrationSteps >> 8) & 0xFF;
      
      ESP_LOGD(TAG, "integrationSteps = %d → paso = %.2f µs", integrationSteps, integrationSteps * 2.78f);
      return 0;
    }
/*
La Ganancia  AGAIN se define en valores de 0 a 10, siendo 0 Gain 0.5x y 10 Gain 512x (Ver Datasheet Pág 36)
*/
    ...
    //ESP_LOGI(TAG, "ASTEP=%d ATIME=%d",astep,atime); // Debería imprimir en el monitor serial
    ...
    short AS7341Component::defaultConfiguration() {
      //tint = ((ATIME + 1 ) x (ASTEP + 1) x2.78us 
      
      std::string cmd;
      // Tiempo de Integración de 50 ms, según los valores usados en el Datasheet (aStep 599 + aTIME 29)
      this->setIntegrationSteps(599); 
      this->setIntegrationTime(59); //DPM: Lo cambié para tener un tiempo de integración de 100 ms
